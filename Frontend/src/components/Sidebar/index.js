...
const Sidebar = () => {
  ...
  const fetchCanvases = async () => {
    try {
      const response = await axios.get('http://localhost:3030/api/canvas/list', {
  ...
  const handleCreateCanvas = async () => {
    try {
      const response = await axios.post('http://localhost:3030/api/canvas/create', {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      console.log(response.data)  
      fetchCanvases();
  ...
  const handleDeleteCanvas = async (id) => {
    try {
      await axios.delete(`http://localhost:3030/api/canvas/delete/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchCanvases();
